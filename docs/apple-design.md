# Apple Design 動效與材質設計規範

> 這份文件是從 Claude Code 的 `apple-design` skill 原樣轉存過來的參考文件，供 Codex（或任何後續接手的 agent）在做任何 UI / 動效 / 手勢相關開發之前先讀過並遵守。內容取材自 Apple 的 WWDC 設計演講（主要是 *Designing Fluid Interfaces*, WWDC 2018），翻譯並對應到 Web 平台（CSS、Pointer Events、`requestAnimationFrame`、Motion/Framer Motion 這類 spring 動畫函式庫)。
